...
from ..types import EligibilityStatus, SyntheticCase
...
class EvalHarness:
    ...
    def run(self, n_per_program: int = 24, ambiguous_ratio: float = 0.25) -> EvalResult:
        generator = SyntheticCaseGenerator(seed=self.settings.seed)
        cases = generator.generate_eval_set(n_per_program=n_per_program, ambiguous_ratio=ambiguous_ratio)
        pipeline = CasePipeline(self.settings)
        records: list[EvalRecord] = []
        for case in cases:
            result = pipeline.run_case(case)
            records.extend(records_for_case(case, result))
        return EvalResult(records=records, report=compute_metrics(records))
