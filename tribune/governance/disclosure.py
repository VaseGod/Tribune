...
def generate_statutory_disclosure_block(
    verification_status: bool = True,
    watermark_score: SynthIDWatermarkScore | None = None,
    c2pa_manifest: C2PAManifest | None = None,
    eu_ai_act_article_50: bool = True,
) -> list[str]:
    """Generate dynamic statutory disclosure block compliant with EU AI Act Article 50 and C2PA standards."""
    lines = [
        "6. STATUTORY AI TRANSPARENCY DISCLOSURE (EU AI ACT ART. 50 & JURISDICTIONAL MANDATES)",
        "   [SYSTEM DESIGNATION]: High-Stakes Automated Administrative Decision Support Assistant.",
        "   [TRANSPARENCY NOTICE]: This text was synthesized and verified by sovereign AI models.",
    ]

    if watermark_score is not None:
        wm_status = "DETECTED" if watermark_score.is_watermarked else "NOMINAL"
        lines.append(
            f"   [WATERMARK ANALYTICS]: SynthID-Text statistical signature {wm_status} "
            f"(z={watermark_score.z_score:.2f}, p_synthetic={watermark_score.p_synthetic:.4f}, "
            f"green_ratio={watermark_score.green_token_ratio:.2%}, confidence={watermark_score.confidence_level})."
        )
    else:
        lines.append("   [WATERMARK ANALYTICS]: Embedded SynthID-Text cryptographic pseudorandom token distribution.")

    if c2pa_manifest is not None:
        lines.extend([
            "",
            "7. C2PA CRYPTOGRAPHIC PROVENANCE & AUDIT MANIFEST",
            f"   [C2PA MANIFEST ID]: {c2pa_manifest.instance_id}",
            f"   [CLAIM GENERATOR]: {c2pa_manifest.claim_generator}",
            f"   [DATA HASH]: SHA-256:{c2pa_manifest.get_data_hash()[:24]}...",
            f"   [DIGITAL SIGNATURE]: SHA-256:{c2pa_manifest.signature_hash[:24]}... [VERIFIED]",
            f"   [TIMESTAMP UTC]: {c2pa_manifest.signed_at}",
        ])

    lines.extend([
        "",
        "8. LEGAL NOTICE & HUMAN OVERSIGHT",
...
def generate_determination_notice(
    ...
    findings and statutory citations via the ActionGate. Dynamically embeds EU AI Act Article 50
    statutory disclaimers and C2PA cryptographic audit blocks.
    """
    ...
    # Auto-generate watermark score if not provided
    preliminary_text = "\n".join(lines)
    effective_wm = watermark_score
    if effective_wm is None:
        detector = SynthIDTextWatermarkDetector()
        effective_wm = detector.compute_watermark_score(preliminary_text)

    # Auto-generate C2PA manifest if required
    effective_manifest = c2pa_manifest
    if effective_manifest is None and attach_c2pa_audit:
        cit_sources = [c.source for c in assessment.citations]
        effective_manifest = sign_c2pa_manifest(
            document_text=preliminary_text,
            case_id=assessment.case_id,
            statutory_citations=cit_sources,
        )

    # Attach dynamic statutory transparency and cryptographic audit blocks
    disclosure_lines = generate_statutory_disclosure_block(
        verification_status=is_cert,
        watermark_score=effective_wm,
        c2pa_manifest=effective_manifest,
    )
